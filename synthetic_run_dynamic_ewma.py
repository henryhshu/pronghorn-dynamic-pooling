...
filename = "data/"
if sys.argv[3] == "pypy": 
  filename += "python-" + test + "-dynamic-ewma.csv"
else:
   filename += "java-" + test + "-dynamic-ewma.csv"
BENCHMARKS = sys.argv[5:]

# -----------------------------------------------------------------------
# Strategy configurations for comparative analysis.
#
# We include the same baselines that run.sh uses *plus* the new
# dynamic_ewma strategy so that all results end up in the same CSV
...
if sys.argv[3] == "pypy": 
  logging.basicConfig(filename="logs/python-" + test + "-dynamic-ewma.log", format='%(asctime)s %(filename)s: %(message)s', filemode='a+')
else:
   logging.basicConfig(filename="logs/java-" + test + "-dynamic-ewma.log", format='%(asctime)s %(filename)s: %(message)s', filemode='a+')
                ...
                for index, request in tqdm(enumerate(range(NUM_REQUESTS))):
                  for retry in range(3):
                    ...
                    try:
                      ...
                      # CSV format identical to synthetic_run.py for comparative analysis:
