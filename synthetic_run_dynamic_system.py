...
filename = "data/"
if sys.argv[3] == "pypy": 
  filename += "python-" + test + "-dynamic-system.csv"
else:
   filename += "java-" + test + "-dynamic-system.csv"
BENCHMARKS = sys.argv[5:]

# -----------------------------------------------------------------------
# Strategy configurations for comparative analysis.
#
# We include the same baselines (cold, fixed, request_centric) that
# run.sh uses *plus* the new dynamic_system strategy so that all
# results end up in the same CSV and can be compared directly.
#
# The dynamic_system strategy string is parsed by cr_deserialize()
# in agent-python/orchestration/utils.py.  Additional parameters
# (max_pool_size, min_pool_size, etc.) use their defaults unless
# appended to the ENV string.
# -----------------------------------------------------------------------
STRATEGIES = [
    "cold",
...
if sys.argv[3] == "pypy": 
  logging.basicConfig(filename="logs/python-" + test + "-dynamic-system.log", format='%(asctime)s %(filename)s: %(message)s', filemode='a+')
else:
   logging.basicConfig(filename="logs/java-" + test + "-dynamic-system.log", format='%(asctime)s %(filename)s: %(message)s', filemode='a+')
                ...
                for index, request in tqdm(enumerate(range(NUM_REQUESTS))):
                  for retry in range(3):
                    ...
                    try:
                      ...
                      # CSV format is identical to synthetic_run.py for comparative analysis:
                      # index, benchmark, mutability, strategy, rate, client_side, server_side, overhead
